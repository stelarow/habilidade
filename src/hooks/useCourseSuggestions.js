/**
 * useCourseSuggestions Hook - Hook personalizado para sugest�es de cursos
 * Integra com o sistema de matching e gerencia estado das sugest�es
 */
...
/**
 * Hook para obter sugest�es de cursos baseadas em post de blog
 */
export const useCourseSuggestions = (post, options = {}) => {
  ...
  const fetchSuggestions = useMemo(() => {
    return async (forceRefresh = false) => {
      ...
      try {
        ...
        setError(error_.message || 'Erro ao buscar sugest�es de cursos');
        setSuggestions([]);
      } finally {
        setIsLoading(false);
      }
  ...
  // Fun��o para for�ar refresh
  const refresh = () => {
    fetchSuggestions(true);
  };

  // M�tricas das sugest�es
...
/**
 * Hook para sugest�es em tempo real durante digita��o (para admin)
 */
...
/**
 * Hook para comparar mudan�as em sugest�es
 */
...
/**
 * Hook para cache local de sugest�es
 */
export const useCachedCourseSuggestions = (post, options = {}) => {
  ...
  // Atualiza cache quando h� novas sugest�es
  useEffect(() => {
