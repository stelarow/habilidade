...
function removeDuplicateContent(content, postTitle) {
  ...
  // This covers the complete image structure generated by markdown processor
  const firstImgRegex = /<div class="mb-6">\s*<img[^>]*>\s*<div class="hidden[^>]*>[\s\S]*?<\/div>\s*(<p class="text-gray-500 text-sm text-center mt-2 italic">[^<]*<\/p>)?\s*<\/div>/i;
  processedContent = processedContent.replace(firstImgRegex, '');
  
  // Fallback: remove any remaining first image structure if the above didn't match
  if (processedContent.includes('<img')) {
    const fallbackImgRegex = /<div[^>]*class="[^"]*mb-6[^"]*"[^>]*>[\s\S]*?<img[^>]*>[\s\S]*?<\/div>/i;
    processedContent = processedContent.replace(fallbackImgRegex, '');
  }
