...
const DesignGraficoBackground: React.FC<DesignGraficoBackgroundProps> = ({ 
  ...
  const GeometricShapeRef = useRef<any>(null);
  const BrushStrokeRef = useRef<any>(null);
  ...
  useMemo(() => {
    ...
    class GeometricShape {
      ...
      constructor(canvas: HTMLCanvasElement, configRef: typeof config) {
        ...
        this.vx = (Math.random() - 0.5) * this.config.shapeSpeed;
        this.vy = (Math.random() - 0.5) * this.config.shapeSpeed;
        this.size = 20 + Math.random() * 60;
        this.rotation = Math.random() * Math.PI * 2;
        this.rotationSpeed = (Math.random() - 0.5) * 0.05;
        this.opacity = 0.1 + Math.random() * 0.3;
        this.type = this.config.shapeTypes[Math.floor(Math.random() * this.config.shapeTypes.length)];
      ...
      update() {
        if (this.config.shapeSpeed === 0) return;
      ...
      draw(ctx: CanvasRenderingContext2D) {
        ...
        const colors = [this.config.colors.gradient1, this.config.colors.gradient2, this.config.colors.gradient3];
    ...
    class BrushStroke {
      ...
      constructor(canvas: HTMLCanvasElement, configRef: typeof config) {
        ...
        this.x = Math.random() * canvas.width;
        this.y = Math.random() * canvas.height;
        this.vx = (Math.random() - 0.5) * this.config.brushSpeed;
        this.vy = (Math.random() - 0.5) * this.config.brushSpeed;
        this.points = [];
        this.maxPoints = 8 + Math.floor(Math.random() * 12);
        this.opacity = 0.2 + Math.random() * 0.4;
        this.width = 2 + Math.random() * 8;
        this.color = [this.config.colors.gradient1, this.config.colors.gradient2, this.config.colors.gradient3][Math.floor(Math.random() * 3)];
        this.life = 1;
      ...
      update() {
        if (this.config.brushSpeed === 0) return;
    ...
    GeometricShapeRef.current = GeometricShape;
    BrushStrokeRef.current = BrushStroke;
  }, [config]);

  // Store drawing functions in useRef to avoid recreating them
  const drawDynamicGradientRef = useRef<((ctx: CanvasRenderingContext2D) => void) | null>(null);
  const drawCursorEffectRef = useRef<((ctx: CanvasRenderingContext2D) => void) | null>(null);

  // Initialize drawing functions only once
  useMemo(() => {
    // Desenhar gradiente dinâmico de fundo
    drawDynamicGradientRef.current = (ctx: CanvasRenderingContext2D) => {
    ...
    drawCursorEffectRef.current = (ctx: CanvasRenderingContext2D) => {
  ...
  const initializeElements = useCallback((canvas: HTMLCanvasElement) => {
    ...
    if (GeometricShapeRef.current) {
      for (let i = 0; i < config.shapeCount; i++) {
        shapesRef.current.push(new GeometricShapeRef.current(canvas, config));
      }
    }
    ...
    if (BrushStrokeRef.current) {
      for (let i = 0; i < config.brushCount; i++) {
        brushStrokesRef.current.push(new BrushStrokeRef.current(canvas, config));
      }
    }
  }, [config]);
  ...
  const animate = useCallback(() => {
    ...
    if (drawDynamicGradientRef.current) {
      drawDynamicGradientRef.current(ctx);
    }
    if (drawCursorEffectRef.current) {
      drawCursorEffectRef.current(ctx);
    }
