    ...
    if (request.nextUrl.pathname.startsWith('/auth/') && !request.nextUrl.pathname.includes('/callback')) {
      ...
      if (user) {
        ...
        if (profile?.role === 'admin') {
          console.log(`[UPDATE_SESSION-${sessionId}] Admin user accessing auth - redirecting to /admin`)
          return NextResponse.redirect(new URL('/admin', request.url))
        } else {
          console.log(`[UPDATE_SESSION-${sessionId}] Regular user accessing auth - redirecting to /dashboard`)
          return NextResponse.redirect(new URL('/dashboard', request.url))
        }
      } else {
