...
export const sendLeadMagnetEmail = async (params: {
  ...
  try {
    const templateParams: EmailParams = {
      ...
      message: `Obrigado por baixar "${params.leadMagnetTitle}". Seu material est� anexado ou dispon�vel no link abaixo.`,
...
export const sendNewsletterConfirmation = async (params: {
  ...
  try {
    const templateParams: EmailParams = {
      ...
      subject: 'Bem-vindo(a) � Newsletter Stelarow Habilidade!',
      message: 'Obrigado por se inscrever em nossa newsletter. Voc� receber� conte�do exclusivo regularmente.',
      from_name: 'Stelarow Habilidade',
    ...
    switch (params.type) {
      case 'new_lead':
        ...
        message = `
          ...
          Nome: ${params.data.name || 'N�o informado'}
          Material: ${params.data.leadMagnetTitle}
          Tipo: ${params.data.leadMagnetType}
          Data: ${new Date().toLocaleString('pt-BR')}
          
          CTA ID: ${params.data.ctaId}
          P�gina: ${params.data.pageUrl}
        `;
        break;
        
      case 'new_subscriber':
        subject = 'Nova Inscri��o Newsletter';
        message = `
          Nova inscri��o na newsletter:
          
          Email: ${params.data.email}
          Nome: ${params.data.name || 'N�o informado'}
          Prefer�ncias: ${params.data.preferences?.join(', ') || 'Nenhuma'}
      ...
      case 'high_demand_cta':
        ...
        message = `
          ...
          Convers�es (�ltimas 24h): ${params.data.conversions}
          Taxa de convers�o: ${params.data.conversionRate}%
          
          Considere ampliar a promo��o ou criar CTAs similares.
        `;
        break;
    }
