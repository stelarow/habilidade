...
version: alpha
name: Bugatti
description: An austere luxury-automotive interface adapted for Escola Habilidade: near-pure black canvas, uppercase letterspaced display, restrained serif body copy, and a controlled Habilidade purple accent. The system runs three Bugatti-inspired type roles - Display, Text Regular, and Monospace - at modest weights with wide tracking to feel engineered, hyper-minimal, and quietly expensive. The purple is allowed only as precision voltage on titles, CTA borders/fills, selected states, and key hairlines; no gradients, shadows, decorative chrome, or extra accent colors.
...
colors:
  ...
  on-photo: "#ffffff"
  link: "#c3d9f3"
  habilidade-purple: "#d400ff"
  habilidade-purple-soft: "rgba(212, 0, 255, 0.10)"
  warning: "#d4a017"
  success: "#5fa657"
...
components:
  button-primary:
    ...
    borderColor: "{colors.habilidade-purple}"
    hoverBackgroundColor: "{colors.habilidade-purple}"
...
Bugatti's marketing surface is the most austere interface in luxury automotive: a near-pure black canvas (`{colors.canvas}` — #000000) holding uppercase **letterspaced** display type and full-bleed photography. In the Escola Habilidade adaptation, the system keeps Bugatti's restraint but adds one controlled brand accent: **Habilidade Purple** (`{colors.habilidade-purple}` — #d400ff). The purple is not decoration; it is precision voltage for primary titles, CTA outlines/fills, selected states, progress hairlines, ranking bars, and the most important recommendation border.
...
**Key Characteristics:**
- Pure black canvas (`{colors.canvas}` — #000000) with white/gray type and controlled Habilidade purple accents. The system does not have a light mode.
...
- Habilidade Purple (`{colors.habilidade-purple}` — #d400ff) is the only brand accent. Use it on titles, CTA borders/fills, selected states, and key hairlines only.
- Buttons are pill-shaped (`{rounded.pill}`) with **transparent background** and a 1px Habilidade Purple outline. On hover/active primary CTAs may fill with Habilidade Purple. Do not change thickness, radius, font, or tracking.
...
### Brand & Accent
- **Primary** (`{colors.primary}` — #ffffff): The base brand color. White remains the default text and neutral outline color on the black canvas.
- **Habilidade Purple** (`{colors.habilidade-purple}` — #d400ff): The Escola Habilidade adaptation accent. Use it deliberately on major display titles, primary CTA outlines and hover/fill states, selected controls, progress indicators, ranking bars, and one primary recommendation border. It should feel machined and sparse, not decorative.
- **Habilidade Purple Soft** (`{colors.habilidade-purple-soft}` — rgba(212, 0, 255, 0.10)): A low-opacity hover or focus wash behind outline controls. Do not use it as a section background or card fill.
- **Link** (`{colors.link}` — #c3d9f3): Legacy Bugatti inline-link color. In Habilidade pages, prefer `{colors.habilidade-purple}` for branded CTAs and reserve this ice-blue only for body links when a secondary non-brand link color is needed.
...
**`button-primary`** — The signature primary CTA. Background **transparent**, text `{colors.on-dark}` (white), 1px outline, rounded `{rounded.pill}` (9999px), padding 14px × 32px, height 44px. Type `{typography.button}` — Bugatti Monospace, uppercase, 14px, 2.5px tracking. In the Habilidade adaptation the outline uses `{colors.habilidade-purple}`; the transparent pill remains the button's default posture.

**Escola Habilidade adaptation:** `button-primary` keeps the exact same pill shape, 1px outline, padding, height, font, and tracking, but the outline uses `{colors.habilidade-purple}`. Hover/active states may fill with `{colors.habilidade-purple}` while keeping white text. Do not make the button thicker, larger, bolder, shadowed, or gradient-filled.

**`button-icon`** — Circular icon buttons (carousel arrows, share, language switcher). 40 × 40px, transparent background, 1px outline, rounded `{rounded.full}`. Same outline-only treatment as the primary button; use `{colors.habilidade-purple}` for primary actions and `{colors.hairline-strong}` for secondary actions.

**Escola Habilidade adaptation:** icon buttons may use a `{colors.habilidade-purple}` outline for primary floating actions or key tool controls. Secondary icon buttons stay `{colors.hairline-strong}` until hover.

**`text-link`** — Inline body links may use `{colors.link}` (#c3d9f3) for legacy Bugatti-like editorial links or `{colors.habilidade-purple}` when the link is part of a Habilidade conversion path. Underlined by default. Type inherits `{typography.body-md}` (Bugatti Text Regular, serif).
...
- Keep `{component.button-primary}` transparent with a 1px outline. In Escola Habilidade pages the outline should be `{colors.habilidade-purple}` and may fill purple on hover/active.
- Use `{colors.habilidade-purple}` for primary titles, selected Likert/radio states, progress hairlines, ranking bars, and the single most important recommendation/CTA border.
...
- Keep all purple use flat and precise: no glow, no blur, no gradient, no decorative purple surfaces.

### Don't
- Don't introduce any accent color outside `{colors.habilidade-purple}` and rare `{colors.link}` body links. The Habilidade adaptation is black/white/gray plus one purple.
- Don't bold any type. The system has no bold weight — every typeface stays at 400.
- Don't fill primary buttons by default. Transparent + purple outline is the default; purple fill is reserved for hover/active or selected states.
- Don't use purple gradients, purple shadows, purple particles, purple bokeh, or purple decorative backgrounds. The accent is a line/text/control color, not atmosphere.
...
7. For Escola Habilidade surfaces, apply `{colors.habilidade-purple}` only after the black/white layout is already working. Purple may mark: main display titles, primary CTA outline/fill state, selected controls, key hairlines, ranking bars, and one primary recommendation border.
8. When in doubt about emphasis: stronger hierarchy and whitespace before more purple.
